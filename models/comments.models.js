...
exports.updateArticleComments = async (_id, comment_id, comment) => {
	...
	const updatedUser = await User.findOneAndUpdate(
		...
		{
			$set: {
				"teacher.articles.$[].comments.$[comment].comment_body":
...
exports.sendComment = async (comment, _id) => {
	...
	const result = await User.aggregate([
		...
		{
			$project: {
				lastComment: "$teacher.articles.comments",
				// lastComment: { $arrayElemAt: ["$teacher.articles.comments", -1] },
			},
		},
	]);

	const filteredComment = result[0].lastComment
		.flat()
		.filter((individualComment) => {
			return individualComment.comment_body === comment.comment_body;
		});

	return filteredComment[0];
};
